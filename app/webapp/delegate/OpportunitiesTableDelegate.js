sap.ui.define([
...
], function (ODataTableDelegate, Sorter, FilterField, Field, mdcLibrary, HBox, Button, mLibrary, ComboBox, Item, Input,
    ...
    GenericTableDelegate.addItem = function (oTable, sPropertyName, mPropertyBag) {
        console.log("[GenericDelegate] 🔵 addItem called for property:", sPropertyName, "in table:", oTable.getId());
        ...
        return this.fetchProperties(oTable).then(function (aProperties) {
            ...
            const mCustomHeaders = {
                ...
                "tcv": "TCV",
                "Stage": "Deal Stage",
                "customerId": "Customer Name"  // ✅ FIXED: Use proper name from association config
            };
            ...
            return new Promise(function (resolve) {
                sap.ui.require(["sap/ui/mdc/table/Column"], function (Column) {
                    ...
                    oAssocPromise.then(function(oAssocConfig) {
                        ...
                        // DEBUG: Check association detection
                        console.log("[GenericDelegate] 🔍 Association check for", sPropertyName, ":", {
                            isEnum: bIsEnum,
                            isAssoc: bIsAssoc,
                            assocConfig: oAssocConfig,
                            tableId: sTableId,
                            propertyName: sPropertyName
                        });
                        
                        if (sPropertyName === "customerId") {
                            console.log("[GenericDelegate] 🎯 CUSTOMER ID FIELD DETECTED! Association:", bIsAssoc, "Config:", oAssocConfig);
                        }
                        ...
                        if (bIsEnum) {
                            ...
                            // ✅ ASSOCIATION: Use Input with ValueHelp button + SelectDialog (simple and reliable)
                            const oModel = oTable.getModel();
                            ...
                            // Store reference to the Field (will be set after Field creation)
                            let oParentField = null;
                                    ...
                                    if (!oInput._oSelectDialog) {
                                        const oSelectDialog = new SelectDialog({
                                            ...
                                            confirm: function(oEvent) {
                                                ...
                                                // Method 1: Get resolved path from Input's value binding
                                                // For OData V4, the binding path like "{customerId}" resolves to "/Opportunities('O-0001')/customerId"
                                                ...
                                                if (oInputBinding && oInputBinding.getResolvedPath) {
                                                    try {
                                                        ...
                                                        if (sResolvedPath) {
                                                            // Extract row path: "/Opportunities('O-0001')/customerId" -> "/Opportunities('O-0001')"
                                                            const sRowPath = sResolvedPath.substring(0, sResolvedPath.lastIndexOf("/" + sPropertyName));
                                                            if (sRowPath) {
                                                                oRowContext = oModel.getContext(sRowPath);
                                                                console.log("[ValueHelp] Found context from resolved path:", sRowPath);
                                                            }
                                                        }
                                                    } catch (e) {
                                                ...
                                                // Method 2: Try Field's binding resolved path
                                                if (!oRowContext) {
                                                        ...
                                                        if (oFieldBinding && oFieldBinding.getResolvedPath) {
                                                            try {
                                                                ...
                                                                if (sResolvedPath) {
                                                                    ...
                                                                    if (sRowPath) {
                                                                        oRowContext = oModel.getContext(sRowPath);
                                                                        console.log("[ValueHelp] Found context from Field resolved path:", sRowPath);
                                                                    }
                                                                }
                                                            } catch (e) {
                                                ...
                                                // Method 3: Try binding context directly (works sometimes)
                                                if (!oRowContext && oInputBinding) {
                                                    oRowContext = oInputBinding.getContext();
                                                }
                                                
                                                // Method 4: Walk up parent tree
                                                if (!oRowContext) {
                                                    let oCurrent = oInput;
                                                    while (oCurrent) {
                                                ...
                                                if (!oRowContext) {
                                                    ...
                                                    console.error("[ValueHelp] Input binding:", oInputBinding);
                                                    console.error("[ValueHelp] Input resolved path:", oInputBinding && oInputBinding.getResolvedPath ? oInputBinding.getResolvedPath() : "N/A");
                                                ...
                                                if (aSelectedItems && aSelectedItems.length > 0) {
                                                    ...
                                                    if (oSelectedContext) {
                                                        ...
                                                        // Update the row's property using the fresh context
                                                        oRowContext.setProperty(sPropertyName, sKey);
                                                        // Also update Input value for immediate visual feedback
                                                        oInput.setValue(sDisplay);
                                                        console.log("[ValueHelp] ✅ Updated", sPropertyName, "=", sKey, "(" + sDisplay + ") for row", oRowContext.getPath());
                                                    }
                                                }
                                            }
                                        });
                                        ...
                                        // Add to view for proper lifecycle
